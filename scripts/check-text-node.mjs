...
const checks = [
  ...
  [css.includes('.text-node-editor') && css.includes('font-size:42px'), 'text editor 3x font size'],
  [css.includes('.text-format-bar'), 'format bar CSS'],
  [eng.includes('function openTextNodeExpand') && eng.includes('text-expand-modal'), 'expand editor panel'],
  [eng.includes('function applyTextNodeStageBg') && eng.includes('stageBg'), 'node stage background color'],
  [eng.includes('insertHorizontalRule'), 'divider insert'],
  [eng.includes('function saveTextNodeToAssetLibrary'), 'save text stage to library'],
  [i18n.includes('双击开始编辑') && i18n.includes('开启你的创作'), 'placeholders match reference'],
  [css.includes('.text-format-bg-swatch') && css.includes('.text-expand-panel'), 'bg swatch + expand CSS'],
