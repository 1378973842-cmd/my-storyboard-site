...
- **当前进度**：
  0. **本地：gpt-image-2.5 文生图**：无参考走 `sunburst/text-to-image`；有参考仍走 `sunburst/image-to-image`。未部署。
  0. **本地：灯箱对比多参考切换**：结果对比时若有多张参考，底部出现图1/图2…缩略图，一次仍只跟一张滑块对比。未部署。
...
- **待办**：硬刷新实测 gpt-image-2.5 无参考文生图 / 有参考改图；Seedance 2.0（含 @ 引用）。硬刷新实测工具栏新节点下载原图。框选/蒙版/3D机位；日间选中色是否改钴蓝待拍板；高清放大/抠图仍待实测。gpt-image-2.5 未接扩图/重绘/复刻。
- **已知未覆盖（需导演决定是否继续）**：
